  ...
  return (
    <div className={`h-full flex flex-col transition-all duration-700 ${isZenMode ? 'fixed inset-0 z-50 bg-[#0f0f1a] text-white p-12' : 'p-6 max-w-4xl mx-auto bg-transparent'}`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center space-y-12 relative">
        ...
        {showHistory && (
             <div className="absolute top-0 right-0 h-full w-80 bg-[#1a1a2e] rounded-2xl border border-white/10 shadow-2xl z-20 animate-fade-in p-6 overflow-y-auto custom-scrollbar">
        ...
        <div className="text-center relative">
             <div className={`absolute inset-0 rounded-full blur-3xl opacity-20 ${isActive ? 'bg-indigo-500 animate-pulse' : 'bg-transparent'}`}></div>
             <div className="flex justify-center gap-4 mb-8">
                 <button onClick={() => switchSession('focus')} className={`text-xs uppercase tracking-widest font-bold px-3 py-1 rounded-full transition-colors ${sessionType === 'focus' ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 'text-white/40 hover:text-white'}`}>Focus</button>
                 <button onClick={() => switchSession('shortBreak')} className={`text-xs uppercase tracking-widest font-bold px-3 py-1 rounded-full transition-colors ${sessionType === 'shortBreak' ? 'bg-green-500/20 text-green-300 border border-green-500/30' : 'text-white/40 hover:text-white'}`}>Short Break</button>
                 <button onClick={() => switchSession('longBreak')} className={`text-xs uppercase tracking-widest font-bold px-3 py-1 rounded-full transition-colors ${sessionType === 'longBreak' ? 'bg-blue-500/20 text-blue-300 border border-blue-500/30' : 'text-white/40 hover:text-white'}`}>Long Break</button>
             </div>
             
             <h1 className={`font-mono font-light tracking-widest tabular-nums text-white ${isZenMode ? 'text-9xl' : 'text-8xl'}`}>
                {formatTime(timeLeft)}
             </h1>
        ...
        <div className="flex items-center gap-6">
            <button 
                ...
                className={`w-16 h-16 rounded-full flex items-center justify-center transition-all transform hover:scale-105 ${isActive ? 'bg-amber-500/20 text-amber-400 border border-amber-500/50' : 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-500/40'}`}
            >
            ...
            <button 
                ...
                className={`p-4 rounded-full bg-white/5 hover:bg-white/10 text-white/60 hover:text-white border border-white/10`}
            >
                <RefreshCw size={20} />
            </button>
        </div>
        ...
        <div className={`flex gap-4 p-4 rounded-2xl bg-white/5 border border-white/5`}>
            <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-colors ${activeSound === 'rain' ? 'bg-indigo-600 text-white' : 'text-white/60 hover:bg-white/10'}`}
            >
                <Headphones size={16} /> Rain
            </button>
            <button 
                onClick={() => toggleSound('cafe')}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-colors ${activeSound === 'cafe' ? 'bg-indigo-600 text-white' : 'text-white/60 hover:bg-white/10'}`}
            >
      ...
      {showSettings && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm">
              <div className="bg-[#1a1a2e] rounded-2xl shadow-2xl p-6 w-full max-w-sm m-4 text-white animate-slide-up border border-white/10 ring-1 ring-white/5">
