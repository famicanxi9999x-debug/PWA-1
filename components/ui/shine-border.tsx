...
export function ShineBorder({
  ...
  return (
    <div
      ...
      className={cn(
        "relative grid h-full w-full place-items-center rounded-3xl bg-white p-3 text-black dark:bg-zinc-900 dark:text-white",
        className,
      )}
    >
      <div
...
// Adapted Timeline components to use Lucide Icons instead of dicons to ensure compatibility
const IconMap = {
  Shapes,
  Send,
  Check,
...
export const TimelineContainer: React.FC<{ children: ReactNode }> = ({ children }) => {
  return (
...
export const TimelineEvent: React.FC<Event & { isLast?: boolean }> = ({
  label,
  message,
  icon,
  isLast = false,
